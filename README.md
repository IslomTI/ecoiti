...
! 6. [cite_start]Настройка OSPF (динамическая маршрутизация) [cite: 133-137]
router ospf 1
 router-id 1.1.1.1
 ...
 ! Включаем аутентификацию для зоны 0
...
! 2. Указываем, где живет сервер RADIUS [cite: 101]
! IP сервера srv1-cod
radius-server host 192.168.100.10 key radius_secret

! 3. Создаем список методов аутентификации "default"
! Сначала пробуем group radius, если сервер недоступен - local (локальная база) [cite: 105]
aaa authentication login default group radius local

! 4. Настраиваем локального пользователя (на случай сбоя RADIUS)
username admin privilege 15 secret P@ssw0rdLocal

! В задании сказано: "Для rtr-cod пользователь netuser не должен существовать локально" [cite: 103]
...
! 2. [cite_start]Внешний интерфейс [cite: 83]
interface gigabitethernet 0/0
 description WAN_TO_ISP
...
! [cite_start]Настроим саб-интерфейсы для VLAN (100, 200, 300) [cite: 91]
! VLAN 300 (MGMT)
...
! [cite_start]Но в задании сказано "Настройте... BGP c ISP"[cite: 127]. 
